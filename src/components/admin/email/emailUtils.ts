...
function formatMessageToHTML(message: string): string {
  ...
  formattedMessage = formattedMessage.replace(/`(.*?)`/g, '<code style="background-color: #f1f1f1; padding: 2px 4px; border-radius: 4px; font-family: monospace;">$1</code>');
  
  // Convert links [text](url) to <a href="url">text</a>
  formattedMessage = formattedMessage.replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" style="color: #3B82F6; text-decoration: underline;">$1</a>');
