...
export default function Stepper({
  ...
  const handleNext = () => {
    // Loop back to first step after last step
    if (isLastStep) {
      setDirection(1);
      updateStep(1);
    } else {
...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators = false }: StepIndicatorProps) {
  ...
  return (
    ...
    >
      <motion.div
        variants={{
          inactive: { scale: 1, backgroundColor: '#888', color: '#d0d0d0' },
          active: { scale: 1, backgroundColor: '#FF7043', color: '#FF7043' },
...
function StepConnector({ isComplete }: StepConnectorProps) {
  const lineVariants: Variants = {
    ...
    complete: { width: '100%', backgroundColor: '#FF7043' }
  };
