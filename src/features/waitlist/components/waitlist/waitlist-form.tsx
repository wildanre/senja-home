...
import { WalletConnectButton } from "@/features/waitlist/components/wallet-connect-button";
import { useGuildMembership } from "@/features/waitlist/hooks/use-guild-membership";
import { useWalletMutation } from "@/features/waitlist/hooks/use-wallet-mutation";
import { useWaitlistMutation } from "@/features/waitlist/hooks/use-waitlist-mutation";
import { useWaitlistStatus } from "@/features/waitlist/hooks/use-waitlist-status";
import type { DiscordAuthStatus } from "@/types/auth";
import { LoadingState } from "./loading-state";
import WaitlistCountdown from "./waitlist-countdown";

interface WaitlistFormProps {
  initialAuth: DiscordAuthStatus;
}
...
export default function WaitlistForm({
  ...
  const { user, isAuthenticated, loading, login, checkAuth } =
    useDiscordAuth();
  ...
  const now = Date.now();
  const launchTime = new Date(appConfig.waitlistLaunchDate).getTime();
  const hasLaunched = now >= launchTime;

  // Show countdown if not launched yet
  if (!hasLaunched) {
    return <WaitlistCountdown launchDate={appConfig.waitlistLaunchDate} />;
  }
